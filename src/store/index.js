...
export const store =   createStore({
  ...
  mutations: {
    ...
    addCart(state, food){
      const targetCart = state.cart.find(targetCart => targetCart.userId == state.activeUser.id)
      food.id ?
      targetCart.cart.find(targetFood => targetFood.id == food.id).foodCount+=1
      :
      targetCart.cart.push({count: 1, ...food})
    },
    deleteFromCart(state, food){
      const targetCart = state.cart.find(targetCart => targetCart.userId == state.activeUser.id)
      const targetCartItem = targetCart.userFoods.find(userFoodItem => userFoodItem.id == food.id)
      targetCartItem.id ?
      targetCartItem.foodCount-=1
      :
      targetCart.userFoods.splice(
        targetCart.userFoods.findIndex(targetFood => targetFood.id == food.id)
        ,1)
    },
